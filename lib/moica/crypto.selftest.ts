...
const checksum = computeChecksum(payload, aesKeyBase64);
assert(/^[0-9a-f]+$/.test(checksum), "checksum is lowercase hex");
assert(verifyChecksum(payload, checksum, aesKeyBase64) === true, "verifyChecksum accepts a checksum of the same payload");

// 2. tamper with the payload -> verification must fail.
assert(
  verifyChecksum(payload + "x", checksum, aesKeyBase64) === false,
  "verifyChecksum rejects a mismatched payload"
);

// 3. wrong key -> verification must fail, not throw.
const wrongKey = randomBytes(32).toString("base64");
assert(verifyChecksum(payload, checksum, wrongKey) === false, "verifyChecksum rejects the wrong AES key");
